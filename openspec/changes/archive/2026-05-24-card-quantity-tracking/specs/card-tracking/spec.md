...
- **WHEN** a card is marked as both collected (amount >= 1) and needed
...
- **WHEN** a card is marked as collected (amount >= 1), needed, and wanted simultaneously
...
### Requirement: Collection Quantity Data Model
The CollectedCard model SHALL track the quantity of each card owned using an integer amount field.

#### Scenario: CollectedCard has amount field
- **WHEN** a CollectedCard record exists in the database
- **THEN** it SHALL have an `amount` integer field with a minimum value of 1
- **AND** the default value for new records SHALL be 1

#### Scenario: Card is considered collected
- **WHEN** a CollectedCard record exists for a user+set+card combination
- **THEN** the card is considered collected (amount is always >= 1)
- **AND** if the amount would reach 0, the row SHALL be deleted instead

### Requirement: Collection Toggle API with Quantity Support
The POST `/api/collection/toggle` endpoint SHALL support quantity operations via an optional `action` field.

#### Scenario: Legacy toggle behavior (no action field)
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId }`
- **AND** no `action` field is provided
- **THEN** if no CollectedCard row exists, one is created with amount=1
- **AND** if a CollectedCard row exists, it is deleted

#### Scenario: Increment action
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "increment" }`
- **THEN** if no CollectedCard row exists, one is created with amount=1
- **AND** if a CollectedCard row exists, amount is incremented by 1
- **AND** the response includes `{ collected: true, amount: <new_amount> }`

#### Scenario: Decrement action
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "decrement" }`
- **AND** a CollectedCard row exists with amount > 1
- **THEN** the amount is decremented by 1
- **AND** the response includes `{ collected: true, amount: <new_amount> }`

#### Scenario: Decrement to zero deletes the row
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "decrement" }`
- **AND** a CollectedCard row exists with amount = 1
- **THEN** the CollectedCard row is deleted
- **AND** the response includes `{ collected: false, amount: 0 }`

#### Scenario: Decrement on non-existent card
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "decrement" }`
- **AND** no CollectedCard row exists
- **THEN** no change is made
- **AND** the response includes `{ collected: false, amount: 0 }`

#### Scenario: Set exact amount
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "set", amount: N }`
- **AND** N >= 1
- **THEN** the CollectedCard row is created or updated with amount=N
- **AND** the response includes `{ collected: true, amount: N }`

#### Scenario: Set amount to zero
- **WHEN** an authenticated user sends POST to `/api/collection/toggle` with `{ setId, cardId, action: "set", amount: 0 }`
- **THEN** the CollectedCard row is deleted if it exists
- **AND** the response includes `{ collected: false, amount: 0 }`

### Requirement: Collection Data Response Format
The GET `/api/collection` endpoint SHALL return quantity data instead of boolean data.

#### Scenario: Collection data includes amounts
- **WHEN** an authenticated user sends GET to `/api/collection`
- **THEN** the response is a nested object `{ setId: { cardId: number } }`
- **AND** each cardId value is the amount (integer >= 1)

#### Scenario: Collected status derived from amount
- **WHEN** frontend code checks if a card is collected
- **THEN** a card with amount >= 1 is considered collected
- **AND** a card not present in the response is considered uncollected (amount 0)

### Requirement: Set Completion Progress
Set completion progress SHALL count unique cards collected, not total copies.

#### Scenario: Progress counts unique cards
- **WHEN** calculating completion progress for a set
- **THEN** each card with amount >= 1 counts as 1 toward completion
- **AND** a card with amount=5 still counts as 1 toward completion
- **AND** progress percentage = (unique cards with amount >= 1) / (total cards in set) * 100
